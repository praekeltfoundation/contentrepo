...
class TestImportExport:
    ...
    def test_invalid_wa_template_category(self, csv_impexp: ImportExport) -> None:
        ...
        assert (
            e.value.message
            == "Validation error: Select a valid choice. Marketing is not one of the available choices."
        )
    ...
    def test_invalid_wa_template_vars(self, csv_impexp: ImportExport) -> None:
        ...
        assert (
            e.value.message
            == "Validation error: The number of example values provided (1) does not match the number of variables used in the template (3)"
        )
    ...
    def test_footer_maximum_characters(self, csv_impexp: ImportExport) -> None:
        ...
        assert (
            e.value.message
            == "Validation error: Ensure this value has at most 60 characters (it has 110)."
        )
    ...
    def test_list_items_maximum_num(self, csv_impexp: ImportExport) -> None:
        ...
        assert isinstance(e.value, ImportException)
        assert e.value.row_num == 4
        assert e.value.message == "Validation error: The maximum number of items is 10"
    ...
    def test_list_items_maximum_characters(self, csv_impexp: ImportExport) -> None:
        ...
        assert (
            e.value.message
            == "Validation error: List item (Item no 5 a very long list item) has exceeded maximum character limit of 24"
        )
