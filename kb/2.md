...
**Note:** Asterisk (*) in Required column indicates conditional requirement. In this table, `translation_tag` is optional for the default locale but required when importing content in non-default locales.
...
| `time` | CSV list | No* | Time values (*must match number of page_slugs or use "-") | "0,7,14" |
| `unit` | CSV list | No* | Time units (*must match number of page_slugs or use "-") | "days,days,days" |
| `before_or_after` | CSV list | No* | Timing direction (*must match number of page_slugs or use "-") | "after,after,after" |
| `contact_field` | CSV list | No | Contact field names | "signup_date,,,," |

**Note:** Asterisk (*) in Required column indicates conditional requirement. In this table, `time`, `unit`, and `before_or_after` are not required by default, but when provided, they must contain the same number of values as `page_slugs`. Alternatively, you can use a single "-" value which auto-expands to match the number of pages.
...
```csv
"page-1,page-2,page-3","0,7","days","after","signup_date"  ❌ time has 2 values but page_slugs has 3
```
...
| Column | Type | Required | Description | Example |
|--------|------|----------|-------------|---------|
| `question_type` | string | **YES** | Question type | "categorical_question", "age_question", "multiselect_question", "freetext_question", "integer_question", "year_of_birth_question" |
...
| Column | Type | Required | Description | Example |
|--------|------|----------|-------------|---------|
| `answers` | CSV list | No* | Answer option list (*required for categorical_question and multiselect_question) | "Daily, Weekly, Never" |
| `scores` | CSV list | No* | Score list (*required for categorical_question and multiselect_question, must match number of answers) | "3.0, 2.0, 1.0" |
...
**Note:** Asterisk (*) in Required column indicates conditional requirement. The `answers` and `scores` fields are only required for `categorical_question` and `multiselect_question` types. Other question types (`age_question`, `freetext_question`, `integer_question`, `year_of_birth_question`) do not use these fields.

#### Numeric Question Fields

| Column | Type | Required | Description | Example |
|--------|------|----------|-------------|---------|
| `min` | int | No* | Minimum value (*only for integer_question) | 18 |
| `max` | int | No* | Maximum value (*only for integer_question) | 100 |

**Note:** Asterisk (*) in Required column indicates conditional requirement. The `min` and `max` fields are only used for `integer_question` type.
...
Import process reports progress via queue with different stages:

**Content Pages:**
- 0-5%: Loading and parsing file
- 5-10%: Deleting existing content (if purge mode)
- 10-80%: Importing pages (creating/updating content)
- 80-90%: Linking related pages
- 90-100%: Finalizing

**Ordered Content Sets:**
- 0-10%: Loading and parsing file
- 10-100%: Creating ordered content sets

**WhatsApp Templates & Assessments:**
- 0-5%: Loading and parsing file
- 5-10%: Deleting existing content (if purge mode)
- 10-80%: Importing templates/assessments
- 80-100%: Finalizing
